...
<html>
	<head>
		...
		<link rel="stylesheet" href="plugins/css/components/jumpMsg.css?t=20210521V01">
	</head>
	<body>
<?php
}
		...
		<div class="jumpMsgContainer">
			<div class="jumpMsgInner">
				...
				<div class="jumpMsgContent">
					...
					{
					?>
						<div class="jumpMsgContentArticle JqJumpMsgContentArticle"><?php echo $LPaData['sArticle']; ?></div>
					<?php
					}
					?>
				</div>
								...
								?>
									<td class="jumpMsgBtnCell" style="width:calc(100%/<?php echo count($LPaData['aButton']); ?>);max-width:calc(100%/<?php echo count($LPaData['aButton']); ?>);min-width:calc(100%/<?php echo count($LPaData['aButton']); ?>);">
										...
										if(strpos($LPaButtonData['sClass'],'JqClose') !== false)
										{
											# 關閉
										?>
											<div class="jumpMsgBtn JqJumpMsgBtn WordBreakBreakAll <?php echo (!empty($LPaButtonData['sClass']))?$LPaButtonData['sClass']:''; ?>">
										...
										else if(strpos($LPaButtonData['sClass'],'submit') !== false)
										{
											# submit form
										?>
											<div class="jumpMsgBtn JqJumpMsgBtn WordBreakBreakAll">
												<div class="jumpMsgBtnTxt submit" onclick="$('form[data-form=\'<?php echo $LPnId;?>\']').submit();"><?php echo aCENTER['CONFIRM'];?></div>
											</div>
										<?php
										}
										else
										{
											# 換頁
										?>
											<a class="jumpMsgBtn JqJumpMsgBtn WordBreakBreakAll JqRedirectClose <?php echo (!empty($LPaButtonData['sClass']))?$LPaButtonData['sClass']:''; ?>" href="<?php echo $LPaButtonData['sUrl']; ?>">
												<span class="jumpMsgBtnTxt"><?php echo $LPaButtonData['sText']; ?></span>
											</a>
										<?php
										}
