<?php
	...
	function filter_input_float($sName, $sType, $fDefult = 0.0, $fbit = 2, $Exponential = false, $fMin = -2147483648, $fMax = PHP_INT_MAX) 
	{
		...
		// $filter = ($Exponential == true)? 'FILTER_VALIDATE_FLOAT':'FILTER_UNSAFE_RAW';
		...
		else
		{
			if ( $sType === INPUT_REQUEST ) {
				if ( isset($_POST[$sName]) ) {
					$fFun_Val = filter_input(INPUT_POST, $sName, FILTER_UNSAFE_RAW);
				} else {
					if ( isset($_GET[$sName]) ) {
						$fFun_Val = filter_input(INPUT_GET, $sName, FILTER_UNSAFE_RAW);
					}
				}
			} else {
				$fFun_Val = filter_input($sType, $sName, FILTER_UNSAFE_RAW);
			}
		}
	...
	function filter_input_str($sName, $sType, $sDefult = '', $nString_Limit = 0) 
	{
		...
		if ( $sType === INPUT_REQUEST ) {
			if ( isset($_POST[$sName]) ) {
				$sFun_Val = filter_input(INPUT_POST, $sName, FILTER_UNSAFE_RAW);
			} else {
				if ( isset($_GET[$sName]) ) {
					$sFun_Val = filter_input(INPUT_GET, $sName, FILTER_UNSAFE_RAW);
				}
			}
		} else {
			$sFun_Val = filter_input($sType, $sName, FILTER_UNSAFE_RAW);
		}

		if ( $nString_Limit > 0 ) {
			if ( mb_strlen($sFun_Val,'UTF-8') > $nString_Limit ) {
	...
	function filter_ary_recursive($ary,$aInitialSet = array('_Initial'=>'','nMin'=>-2147483648,'nMax'=>PHP_INT_MAX,'nLimit'=>0))
	{
		foreach($ary as $k => $v)
		{
			...
			elseif(is_string($v))
			{
				...
				{
					if ( mb_strlen($v,'UTF-8') > $aInitialSet['nLimit'] )
					{
						mb_internal_encoding('UTF-8');
