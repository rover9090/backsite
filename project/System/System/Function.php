<?php
	...
	function goImage($aData)
	{
		...
		else
		{
			switch ($nState)
			{
				...
				break;
				case 10:
					$aReturn['error'] = 'typeErr';
				break;
				case 11:
					$aReturn['error'] = 'sizeErr';
				break;
				default:
					$aReturn['error'] = 'error';
				break;
	...
	function AppendUploadDetail($sContext)
	{
		global $aMemberData;
		$aWho = 'NONE';
		if(!empty($aMemberData)) $aWho = $aMemberData;
	...
	function checkImage($aFile)
	{
		...
		if(isset($aFile['error']) && $aFile['error'] != 0)
		{
			return $aFile['error'];
		}

		if($aFile["size"] > 1048576 * 3)## 1048576為1MB
		{
			return 11;
		}
		...
		$tmp = explode("/", $aFile["type"]);
		$sExtension2 = strtolower(end($tmp));	## 取出檔案類型
		if (!isset($aAllowedExts[$sExtension]) || !isset($aAllowedExts[$sExtension2]))
		{
			return 10;
		}
	...
	function fnFloorBit($nNumber,$nBit)
	{
		$nShift = pow(10,$nBit);
		return floor($nNumber*$nShift)/$nShift;
	}
	function fnCeilBit($nNumber,$nBit)
	{
		$nShift = pow(10,$nBit);
		return ceil($nNumber*$nShift)/$nShift;
	}
?>