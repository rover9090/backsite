	...
	class oTransfer
	{
		...
		static function PointCheck($nUid)
		{
			...
			try
			{
				...
				$sLongkey = $aRows['nMoneyTime'].$nUid.(sprintf('%.3f',$aRows['nMoney']) ).self::$MoneyKey;
				$tmp = $sLongkey;
				...
				$sLongkey = $aRows['nWaterTime'].$nUid.(sprintf('%.3f',$aRows['nWater']) ).self::$MoneyKey;
				$sLongkey = md5($sLongkey);
				for($i=0;$i<12;$i++)
				{
		...
		static function PointUpdate($nUid, $aNewPoint, $checkdone = 0)
		{
			...
			foreach($aNewPoint as $k => $v)
			{
				...
				{
					$aShortkey[$k] = '';
					$aLongkey[$k] = md5(NOWTIME . $nUid . sprintf('%.3f',$v) . self::$MoneyKey);
					// echo $k.$v.'|';
					for($i=0;$i<12;$i++)
					{
						$aShortkey[$k] .= $aLongkey[$k][self::$aPrime[$i] - 1];
					}
					$aSQL_Array['n' . $k]		= sprintf('%.3f',$v);
