<?php
	...
	if ($aJWT['a'] == 'INS')
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			$sSQL = 'INSERT INTO '. CLIENT_ACTIVITY_KIND . ' ' . sql_build_array('INSERT', $aSQL_Array );
			...
			if($nLid == 0)
			{
				...
				$sSQL = '	UPDATE '. CLIENT_ACTIVITY_KIND . ' SET ' . sql_build_array('UPDATE', $aSQL_Array ).'
			...
			//log ??
			$aEditLog[CLIENT_ACTIVITY_KIND]['aNew'] = $aSQL_Array;
			$aEditLog[CLIENT_ACTIVITY_KIND]['aNew']['nId'] = $nLastId;
			$aActionLog = array(
				'nWho'		=> (int) $aAdm['nId'],
				'nWhom'		=> (int) 0,
				'sWhomAccount'	=> (string) '',
		...
		$aJumpMsg['0']['sShow'] = 1;
		$aJumpMsg['0']['aButton']['0']['sUrl'] = sys_web_encode($aMenuToNo['pages/client_activity/php/_client_activity_kind_0.php']);
	...
	if ($aJWT['a'] == 'UPT'.$nLid)
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			#紀錄動作 - 更新
			$aEditLog[CLIENT_ACTIVITY_KIND]['aNew'] = $aSQL_Array;
			$aEditLog[CLIENT_ACTIVITY_KIND]['aNew']['nId'] = $aId[$LPsLang];
			$aActionLog = array(
				'nWho'		=> (int) $aAdm['nId'],
				'nWhom'		=> (int) 0,
				'sWhomAccount'	=> (string) '',
	...
	if ($aJWT['a'] == 'DEL'.$nLid)
	{
		...
		{
			$aEditLog[CLIENT_ACTIVITY_KIND]['aOld'] = $aData;
			$aSQL_Array = array(
				'nOnline'		=> (int) 99,
				'nUpdateTime'	=> (int) NOWTIME,
				'sUpdateTime'	=> (string) NOWDATE,
