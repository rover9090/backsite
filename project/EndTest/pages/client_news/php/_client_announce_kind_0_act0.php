<?php
	...
	require_once(dirname(dirname(dirname(dirname(__file__)))).'/inc/lang/'.$aSystem['sLang'].'/client_announce_kind.php');
	#require??
	...
	if ($aJWT['a'] == 'INS')
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			$sSQL = 'INSERT INTO '. CLIENT_ANNOUNCE_KIND . ' ' . sql_build_array('INSERT', $aSQL_Array );
			...
			if($nLid == 0)
			{
				...
				$sSQL = '	UPDATE '. CLIENT_ANNOUNCE_KIND . ' SET ' . sql_build_array('UPDATE', $aSQL_Array ).'
			...
			//log ??
			$aEditLog[CLIENT_ANNOUNCE_KIND]['aNew'] = $aSQL_Array;
			$aEditLog[CLIENT_ANNOUNCE_KIND]['aNew']['nId'] = $nLastId;
			$aActionLog = array(
				'nWho'		=> (int)	$aAdm['nId'],
				'nWhom'		=> (int)	0,
				'sWhomAccount'	=> (string)	'',
		...
		$aJumpMsg['0']['sShow'] = 1;
		$aJumpMsg['0']['aButton']['0']['sUrl'] = sys_web_encode($aMenuToNo['pages/client_news/php/_client_announce_kind_0.php']);
	...
	if ($aJWT['a'] == 'UPT'.$nLid)
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			#紀錄動作 - 更新
			$aEditLog[CLIENT_ANNOUNCE_KIND]['aNew'] = $aSQL_Array;
			$aEditLog[CLIENT_ANNOUNCE_KIND]['aNew']['nId'] = $aId[$LPsLang];
			$aActionLog = array(
				'nWho'		=> (int)	$aAdm['nId'],
				'nWhom'		=> (int)	0,
				'sWhomAccount'	=> (string)	'',
	...
	if ($aJWT['a'] == 'DEL'.$nLid)
	{
		...
		{
			$aEditLog[CLIENT_ANNOUNCE_KIND]['aOld'] = $aData;
			$aSQL_Array = array(
				'nOnline'		=> (int)	99,
				'nUpdateTime'	=> (int)	NOWTIME,
				'sUpdateTime'	=> (string)	NOWDATE,
