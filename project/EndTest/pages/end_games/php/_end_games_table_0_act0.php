<?php
	...
	if ($aJWT['a'] == 'INS')
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			$sSQL = 'INSERT INTO '. END_GAMES_TABLE . ' ' . sql_build_array('INSERT', $aSQL_Array );
			...
			if($nLid == 0)
			{
				...
				$sSQL = '	UPDATE '. END_GAMES_TABLE . ' SET ' . sql_build_array('UPDATE', $aSQL_Array ).'
			...
			//log ??
			$aEditLog[END_GAMES_TABLE]['aNew'] = $aSQL_Array;
			$aEditLog[END_GAMES_TABLE]['aNew']['nId'] = $nLastId;
			$aActionLog = array(
				'nWho'		=> (int)	$aAdm['nId'],
				'nWhom'		=> (int)	0,
				'sWhomAccount'	=> (string)	'',
		...
		$aJumpMsg['0']['sShow'] = 1;
		$aJumpMsg['0']['aButton']['0']['sUrl'] = sys_web_encode($aMenuToNo['pages/end_games/php/_end_games_table_0.php']);
	...
	if ($aJWT['a'] == 'UPT'.$nLid)
	{
		...
		foreach(aLANG as $LPsLang => $LPsText)
		{
			...
			#紀錄動作 - 更新
			$aEditLog[END_GAMES_TABLE]['aNew'] = $aSQL_Array;
			$aEditLog[END_GAMES_TABLE]['aNew']['nId'] = $aId[$LPsLang];
			$aActionLog = array(
				'nWho'		=> (int)	$aAdm['nId'],
				'nWhom'		=> (int)	0,
				'sWhomAccount'	=> (string)	'',
	...
	if ($aJWT['a'] == 'DEL'.$nLid)
	{
		...
		{
			$aEditLog[END_GAMES_TABLE]['aOld'] = $aData;
			$aSQL_Array = array(
				'nOnline'		=> (int)	99,
				'nUpdateTime'	=> (int)	NOWTIME,
				'sUpdateTime'	=> (string)	NOWDATE,
