<?php
	...
	#給此頁使用的url
	$aUrl = array(
		'sSync'	=> sys_web_encode($aMenuToNo['pages/end_games/php/_end_games_setting_0_act1.php']).'&run_page=1',
		'sIns'	=> sys_web_encode($aMenuToNo['pages/end_games/php/_end_games_setting_0_upt0.php']),
		'sDel'	=> sys_web_encode($aMenuToNo['pages/end_games/php/_end_games_setting_0_act0.php']).'&run_page=1',
		'sPage'	=> sys_web_encode($aMenuToNo['pages/end_games/php/_end_games_setting_0.php']),
		'sHtml'	=> 'pages/end_games/'.$aSystem['sHtml'].$aSystem['nVer'].'/end_games_setting_0.php',
	);
	#url結束
	...
	$sSQL = '	SELECT	nId,
			...
			AND		nOnline != 99
					'. $sCondition .'
			ORDER	BY	sName0 ASC '.sql_limit($nPageStart, $aPage['nPageSize']);
	$Result = $oPdo->prepare($sSQL);
	$Result->bindValue(':sLang', $aSystem['sLang'], PDO::PARAM_STR);
	sql_build_value($Result,$aBind);
	sql_query($Result);
