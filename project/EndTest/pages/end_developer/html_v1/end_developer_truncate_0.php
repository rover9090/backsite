...
<form action="<?php echo $aUrl['sAct'];?>" method="POST" data-form="0">
	...
	<div class="Block MarginBottom10">
		<div class="Ipt">
			<input type="password" name="sPassword" value="" placeholder="<?php echo '請輸入管理員密碼';?>">
		</div>
		<div class="BtnAny JqStupidOut" data-showctrl="0"><?php echo CSUBMIT;?></div>
	</div>
	<div class="Block">
		<div class="BlockTit MarginBottom5"> admroot / mmg001 <?php echo '不刪';?></div>
		<div class="BtnAny MarginBottom5 JqCheckAll"><?php echo '全選';?></div>
			...
			<div class="GridBlock">
				...
				foreach($LPaTable as $LPsTable => $LPbTrue)
				{
					...
					<label class="GridBlockList" for="<?php echo $LPsTable;?>">
						...
						{
							echo '<div class="Ipt"><input type="text" name="sUserAccount" value="" placeholder="其他保留會員帳號 請用逗號\'，\'隔開"></div>';
						}
						if ($LPsTable == 'end_manager_data')
						{
							echo '<div class="Ipt"><input type="text" name="sManagerAccount" value="" placeholder="其他保留管理帳號 請用逗號\'，\'隔開"></div>';
						}
						?>
					</label>
					<?php
