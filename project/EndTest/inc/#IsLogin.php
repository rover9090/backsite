	...
	*/
	if ($aSystem['nLogin'] == 1) # 需要判斷登入頁面
	{
		...
		while ($aRows = $Result->fetch(PDO::FETCH_ASSOC))
		{
			...
			$aFolder[$sMenuKey.'_upt0']['sListName0'] = $aFolder[$sMenuKey]['sListName0'];
			$aFolder[$sMenuKey.'_upt0']['sListName0'] = $sTempText.$aFolder[$sMenuKey]['sListName0'];
		}
	...
	if($nS === 1 && !empty($aJWT['a']) && !empty($aJWT['t']) && $aJWT['t'] < NOWTIME - 600)
	// if(true)
	{
		$aJumpMsg['0']['sTitle'] = ERRORMSG;
		$aJumpMsg['0']['sIcon'] = 'error';
		$aJumpMsg['0']['sMsg'] = TIMEOUT;
		$aJumpMsg['0']['sShow'] = 1;
		$aJumpMsg['0']['aButton']['0']['sUrl'] = $_SERVER['HTTP_REFERER'];
		$aJumpMsg['0']['aButton']['0']['sText'] = CONFIRM;
	}
	else if (isset($aRequire['Require']) && $bFind == true)
	{
